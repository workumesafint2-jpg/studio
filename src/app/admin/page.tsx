      ...
      <div className="flex items-center justify-center h-screen bg-slate-50">
        <div className="text-center p-10 bg-white rounded-3xl shadow-xl border border-red-100 max-w-md">
          ...
          <div className="flex flex-col gap-3 mt-8">
            <Button asChild className="rounded-xl bg-[#1e3a8a] px-8 py-6 font-bold shadow-lg">
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 bg-slate-50 min-h-screen">
        <div className="flex items-center justify-between mb-8">
          ...
          <header className="flex flex-col items-center gap-1">
            ...
            <p className="text-[10px] font-bold text-slate-400">innovate.smart.app | Master Panel</p>
          </header>
          <div className="w-24" />
        </div>
        ...
        <Card className="shadow-xl border-none overflow-hidden rounded-2xl">
          ...
          <CardContent className="p-0">
            {docsLoading ? (
              <div className="flex justify-center py-24"><Activity className="w-8 h-8 animate-spin text-primary/20" /></div>
                ...
                {allDocs.map((docItem) => (
                  <div key={docItem.id} className="flex items-center justify-between p-4 hover:bg-slate-50/50 transition-colors group">
                    ...
                    <div className="flex items-center gap-3">
                      <Badge variant="outline" className={`text-[7px] border-none px-2 h-4 font-black ${docItem.status === 'የጸደቀ' ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'}`}>
                        {docItem.status}
                      </Badge>
